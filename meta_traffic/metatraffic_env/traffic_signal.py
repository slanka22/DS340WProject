...
class TrafficSignal:
    ...
    def __init__(
        ...
        self.next_action_time = begin_time + delta_time
    ...
    def _build_phases(self):
        ...
        for r, p in enumerate(self.green_phases):
            ...
            for n, i in enumerate(p.state):
                if i == 'G' or i == 's':
                    links = self.sumo.trafficlight.getControlledLinks(self.id)[n][0]
    ...
    def time_to_act(self):
        ...
        # print(self.next_action_time, self.env.sim_step, 'xxxx')
    ...
    def get_accumulated_waiting_time_per_lane(self) -> List[float]:
        ...
        for lane in self.lanes:
            ...
            for veh in veh_list:
                ...
                if veh not in self.env.vehicles_sumo:
                    self.env.vehicles_sumo[veh] = {veh_lane: acc}
                else:
                    self.env.vehicles_sumo[veh][veh_lane] = acc - sum(
                        [self.env.vehicles_sumo[veh][lane] for lane in self.env.vehicles_sumo[veh].keys() if lane != veh_lane]
                    )
                wait_time += self.env.vehicles_sumo[veh][veh_lane]
    ...
    def get_total_emission(self):
        #g/s
        return sum([self.sumo.lane.getCO2Emission(lane) for lane in self.lanes])/1000

    def get_mean_emission(self):
        #g/s
        a = sum([self.sumo.lane.getCO2Emission(lane) for lane in self.lanes])/1000
        n_t = sum([self.sumo.lane.getLastStepVehicleNumber(lane) for lane in self.lanes])
        return a/n_t if n_t else 0
    
    def get_total_travel_time(self):
        # return sum([traci.lane.getTraveltime (lane) for lane in self.lanes])
        v_wa = sum([self.sumo.lane.getLastStepMeanSpeed(lane)*self.sumo.lane.getLastStepVehicleNumber(lane) for lane in self.lanes])
        n_t = sum([self.sumo.lane.getLastStepVehicleNumber(lane) for lane in self.lanes])
        l_t = [self.sumo.lane.getLength(lane) for lane in self.lanes][0] # only the ingoing lane
        return n_t*l_t/v_wa if v_wa>1 else 0
    
    def get_total_delay(self):
        return sum([1 - self.sumo.lane.getLastStepMeanSpeed(lane) / self.sumo.lane.getMaxSpeed(lane) for lane in self.lanes])
