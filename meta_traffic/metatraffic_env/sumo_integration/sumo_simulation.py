...
SumoActor = collections.namedtuple('SumoActor', 'type_id vclass transform signals extent color speed')
    ...
    def __init__(self):
        ...
        for tlid in traci.trafficlight.getIDList():
            ...
            for tllogic in traci.trafficlight.getAllProgramLogics(tlid):
                ...
                if LIBSUMO:
                    parameters = tllogic.subParameter
                else:
...
def _get_sumo_net(cfg_file):
    ...
    sumo_net = sumolib.net.readNet(net_file, withInternal=True, withPedestrianConnections=True, withPrograms=True)
...
class SumoSimulation(object):
    ...
    def __init__(self, cfg_file, step_length,
        ...
        self.net.move(-center_x, -center_y)
        ...
        # Structures to keep track of the spawned and destroyed pedestrian at each time step.
        self.spawned_ped_actors = set()
        self.destroyed_ped_actors = set()
    ...
    def get_actor(actor_id):
        ...
        transform = (Vec3(location[0], location[1], location[2]), Vec3(rotation[0], rotation[1], rotation[2]))

        signals = results[traci.constants.VAR_SIGNALS]
        extent = Vec3(length / 2.0, width / 2.0, height / 2.0)

        speed = results[traci.constants.VAR_SPEED]

        return SumoActor(type_id, vclass, transform, signals, extent, color, speed)

    @staticmethod
    def get_actor_pedestrian(actor_id):
        """
        Accessor for sumo actor.
        """
    ...
    def spawn_actor(self, type_id, color=None):
        ...
        """
        actor_id = 'metadrive_' + str(self._sequential_id)
        try:
            ...
            if vclass not in self._routes:
                ...
                if allowed_edges:
                    traci.route.add("metadrive_route_{}".format(vclass), [allowed_edges[0].getID()])
                    self._routes.add(vclass)
                else:
                    logging.error('Could not found a route for %s. No vehicle will be spawned in sumo', type_id)
                    return INVALID_ACTOR_ID

            traci.vehicle.add(actor_id, 'metadrive_route_{}'.format(vclass), typeID=type_id)
    ...
    def synchronize_vehicle(self, vehicle_id, transform, signals=None):
        ...
        """
        loc_x, loc_y = transform[0].x, transform[0].y
        yaw = transform[1].y
    ...
    def tick(self):
        ...
        # pedestrains
        self.spawned_ped_actors = set(traci.simulation.getDepartedPersonIDList())
        self.destroyed_ped_actors = set(traci.simulation.getArrivedPersonIDList())
