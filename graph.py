...
def graph(ax, data, graph_func, xtitle=None, ytitle_pad=None, title=None, legend=None, grid=None, xlim=None, ylim=None, colours=None):
    ...
    if legend:
        ...
        if colours is None:
            ax.legend(elements, labels, loc=legend, framealpha=1.0, bbox_to_anchor=(1, 0.5))
        else:
...
if __name__ == '__main__':
    ...
    import glob
    file_list = glob.glob('outputs/**/*ep1.csv',recursive=True)
    

    filter_list = ['fixed', 'maxpressure', 'sotl']
    files = [[] for i in range(len(filter_list))]   

    for f in file_list:
        df = pd.read_csv(f, sep=',')
        for i in range(len(filter_list)):
             if filter_list[i] in f and len(files[i])<3 and df['system_total_stopped'].shape[0] == 20000:
                files[i].append(df['system_total_stopped'])
                print(len(files[i]), df['system_total_stopped'].shape)
    
    # print(files)
    box_data = np.array(files)[:,:,:]
    box_data = box_data.reshape((box_data.shape[0], -1)).transpose()
    print(box_data.shape)

    colors = ['b','r','g']
    data_labels = ['fixed', 'maxpressure', 'sotl']
    
    f, axarr = plt.subplots(1,1,figsize=(20,8))
    # box_data = np.random.rand(100, 4)
    graph( axarr, box_data, boxplot( axarr, box_data, colors, data_labels), xtitle='Traffic Signal Controller' , ytitle_pad = ('system total stopped\r\n($\mu$, $\sigma$, median)',90) , title='Traffic Signal Controller\r\nTravel Time', legend='center left', grid=True)
...
#     f, axarr = plt.subplots(1,1)
#     box_data = np.random.rand(100, 4)*2000
