...
parser.add_argument('--datadir', type=str,
                    help='Directory where datasets are stored (rootdir from generation script)')
...
train_loader = DataLoader(
    ...
    batch_size=BSIZE, num_workers=16, shuffle=True, pin_memory=True)
test_loader = DataLoader(
    RolloutSequenceDataset(args.datadir, SEQ_LEN, transform, train=False, buffer_size=4),
...
def to_latent(obs, next_obs):
    ...
    """
    with torch.no_grad():
        # i dont think this is necessary, it might be for the next function
        # but why allocate memory for something that is not used

        obs, next_obs = [
            f.upsample(x.view(-1, 3, SIZE, SIZE).float(), size=RED_SIZE,
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    for i, data in enumerate(loader):
        ...
        # add channel dim
        action = action.unsqueeze(-1).expand(-1, -1, 4)
