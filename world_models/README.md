...
* **--datadir** : The directory in which the data is stored. 

### 1. Data generation
Before launching the VAE and MDN-RNN training scripts, you need to generate a dataset of random rollouts and place it in the `datasets/trafficenv` folder.

Data generation is handled through the `data/generation_script.py` script, e.g.
```bash
python world_models/data/generation_script.py --rollouts 1000 --rootdir datasets/trafficenv --threads 8
```
...
```bash
python world_models/trainvae.py --logdir world_models/exp_dir --datadir datasets/trafficenv
```

### 3. Training the MDN-RNN
The MDN-RNN is trained using the `trainmdrnn.py` file, e.g.
```bash
python world_models/trainmdrnn.py --logdir world_models/exp_dir --datadir datasets/trafficenv
```
A VAE must have been trained in the same `world_models/exp_dir` for this script to work.
### 4. Training and testing the Controller
Finally, the controller is trained using CMA-ES, e.g.
```bash
python world_models/traincontroller.py --logdir world_models/exp_dir --n-samples 4 --pop-size 4 --target-return 950 --display
```
You can test the obtained policy with `test_controller.py` e.g.
```bash
python world_models/test_controller.py --logdir world_models/exp_dir
```

### Notes
When running on a headless server, you will need to use `xvfb-run` to launch the controller training script. For instance,
```bash
xvfb-run -a -s "-screen 0 1400x900x24" python world_models/traincontroller.py --logdir world_models/exp_dir --n-samples 4 --pop-size 4 --target-return 950 --display
```
